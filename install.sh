...
#     environment, rather than left to a one-off export.
#
# Recommended:
...
#: Set by persist_env on first use, for the closing summary.
ENV_SCRIPT=""

# Persist an environment variable into the environment itself.
#
# `conda env config vars set` is the obvious way, and is what this script
# used until it turned out to need the `conda` executable on PATH -- which
# a mamba- or micromamba-only install does not have. Writing an activate.d
# script is equivalent and portable: conda, mamba and micromamba all source
# $CONDA_PREFIX/etc/conda/activate.d/*.sh on activation.
#
# Pass the value ALREADY QUOTED, so deferred expansions survive into the
# generated file:
#     persist_env FOO "\"literal\""
#     persist_env BAR "\"\$SOMETHING_EXPANDED_AT_ACTIVATION\""
persist_env() {
    local script="$CONDA_PREFIX/etc/conda/activate.d/zz-quenais.sh"
    mkdir -p "$(dirname "$script")"
    [ -f "$script" ] || printf '%s\n' \
        '# Auto-generated by QuEnAIS install.sh -- do not edit.' > "$script"
    # Drop any previous definition of this variable, so re-running the
    # installer updates the line rather than appending a second one.
    grep -v "^export $1=" "$script" > "$script.tmp" || true
...
[ -f external/pyci/setup.py ] || [ -f external/pyci/Makefile ] \
  ...
  Editing .gitmodules alone is not enough -- git also needs a gitlink in
  the index, which only 'git submodule add' creates:
...
# LD_LIBRARY_PATH is written with a deferred expansion, so it prepends to
# whatever the surrounding shell has at activation time rather than baking
# in whatever it happened to be during install.
persist_env LD_LIBRARY_PATH "\"$CONDA_PREFIX/lib\${LD_LIBRARY_PATH:+:\$LD_LIBRARY_PATH}\""
persist_env MPI_PATH        "\"$CONDA_PREFIX\""

# Apply to THIS shell as well, so the verification in step 16 can import
# cudaq without reactivating the environment first.
export LD_LIBRARY_PATH="$CONDA_PREFIX/lib${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}"
export MPI_PATH="$CONDA_PREFIX"
ok "LD_LIBRARY_PATH and MPI_PATH persisted to $ENV_SCRIPT"
...
persist_env CUDAQ_DEFAULT_SIMULATOR "\"$CUDAQ_TARGET\""
persist_env WANDB_MODE              "\"$WANDB_MODE_VALUE\""
