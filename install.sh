...
# Persist an environment variable into the env itself.
#
# `conda env config vars set` is the obvious way and was what this script
# used, but it needs the `conda` executable on PATH -- and a mamba- or
# micromamba-only install does not have one. Writing an activate.d script
# is equivalent and portable: conda, mamba and micromamba all source
...
persist_env() {
    ...
    # Drop any previous definition, so re-running updates rather than appends.
